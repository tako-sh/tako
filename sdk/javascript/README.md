...
- Server-only signed optimized image URLs via `createImageUrl` from `tako.sh/server`
...
The `tako.sh` package's named exports include the `tako` runtime object, definition helpers (`defineChannel`, `defineWorkflow`), the workflow `signal` function, `TakoError`, and types (`InferChannel`, `InferWorkflowPayload`, `TakoErrorCode`, `EnqueueOptions`, `WorkflowOpts`). Server-only helpers such as `createImageUrl` live under `tako.sh/server`. There is no `Tako` global — channels and workflows are plain ES modules that you import where you use them.
...
Use `createImageUrl` in server-side code to generate signed optimizer URLs. In isomorphic frameworks, call it inside a loader/server function and pass the signed string to React components:
