...
`tako.d.ts` is generated under the JavaScript `app_root` from `tako.toml` (`src` by default). It augments `tako.sh` with typed secrets, environment names, channel metadata, workflow metadata, and environment-narrowed `process.env` types. App code imports runtime values from `tako.sh`, not from the generated file.
