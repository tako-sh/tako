...
`tako.gen.ts` is generated in your project root (or `src/`) and exports `tako.env`, `tako.secrets`, `tako.logger`, `tako.port`, `tako.host`, `tako.build`, `tako.dataDir`, plus environment-narrowed `process.env` augmentation. The individual runtime values are also exported for existing code, but examples should prefer `tako`.
