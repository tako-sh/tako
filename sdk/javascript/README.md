...
- Runtime state from the generated `tako` object in your project-local `tako.gen.ts` — typed, discoverable, and regenerated by `tako init` / `tako dev` / `tako deploy`
...
`tako.gen.ts` is generated under the JavaScript `app_root` from `tako.toml` (`src` by default) and exports `tako.env`, `tako.secrets`, `tako.logger`, `tako.port`, `tako.host`, `tako.build`, `tako.dataDir`, plus environment-narrowed `process.env` augmentation. The individual runtime values are also exported for existing code, but examples should prefer `tako`.
...
const avatarFallback = createImageUrl("/avatars/u_123.png", {
  width: 256,
  format: "webp",
});
...
Private image URLs are the default and use maximum width `1200`, quality `75`, a 7-day expiration, and 7-day browser-only caching when options are omitted. Use `browserCacheMaxAgeSeconds` only on private URLs when you need a different browser cache max-age. Output defaults to AVIF by omitting `format`; pass `format: "webp"` only when you need a WebP fallback. Tako never upscales: heightless output width is `min(width, originalWidth)`. Pass optional `height` with an explicit `width` to request a bounded box: `fit` defaults to `"cover"`, `crop` defaults to `"center"`, `crop: "smart"` uses libvips attention cropping, and `fit: "contain"` rejects crop. If resize or encode work fails after a verified image source has loaded, Tako serves the original image bytes. Pass `public: true` only for non-user-specific images that can be shared by public caches. Public image options do not accept browser cache or expiration settings. The URL is path-based under `/_tako/image/v1/<payload>.<signature>` and includes no query string.
