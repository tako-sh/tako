...
The interactive status panel always shows local routes plus LAN and tunnel state. LAN and tunnel rows include enable/disable hints, and tunnel shows a starting state while it connects. Inactive tunnel URLs show a Tako error page in browsers and machine-readable errors for API clients.
