...
Creates `tako.toml`, updates `.gitignore`, detects runtime and preset, asks for app name and production route, and installs the SDK package for JS and Go projects. If the production route is a wildcard route, init can collect and encrypt the Cloudflare credential needed for Let's Encrypt DNS-01.
...
The interactive status panel always shows local routes plus LAN and tunnel state. LAN and tunnel rows include their own enable/disable hints on the same rows; tunnel also shows a starting state while it connects.
...
For prebuilt native artifacts, set `start = ["./app"]` and produce that executable in `[build]` or `[[build_stages]]`. Without `start`, deploy uses the selected runtime/preset launch defaults and warns.

For `container = "Dockerfile"`, deploy packages source and the server builds/runs the image with Podman. The container must listen on `$PORT` (`3000` today), bind `0.0.0.0`, and use the Tako SDK so `/status` echoes the internal health-probe token.
