...
```text
<app>-<nonce>.tako.website
```

The nonce is generated by the tunnel service, so tunnel mode does not require login or namespace setup. Tunnels expire after the service TTL, currently 30 minutes, and are also turned off when the app stops or the local tunnel connection closes. When a tunnel is disabled, Tako prints `Tunnel turned off`.
