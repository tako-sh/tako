...
Second, secrets are part of the platform instead of scattered `.env` files. [`tako secrets set`](/docs/cli) stores encrypted project secrets locally, syncs them to mapped servers, and `tako-server` injects them into app processes without writing a release `.env` file. For Next.js apps, generated `tako.gen.ts` gives typed runtime state and typed secrets, so server code can import what it needs without leaning on untyped `process.env` reads.

Third, the backend pieces are moving closer to the app. Tako already has [durable channels](/blog/durable-channels-built-in) for WebSocket/SSE streams and [durable workflows](/blog/durable-workflows-are-here) for retries, cron, sleeps, and `signal` / `waitFor`. A Next.js route can enqueue a workflow or publish to a channel once the server runtime is initialized. That means a self-hosted Next.js app can grow real-time and background work without immediately buying three more services.
