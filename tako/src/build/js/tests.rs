...
#[test]
fn write_tako_gen_module_creates_tako_gen_with_no_secrets() {
    let dir = TempDir::new().unwrap();
    let written = write_tako_gen_module(dir.path()).unwrap();
    assert!(written);

    let content = fs::read_to_string(dir.path().join("src").join("tako.gen.ts")).unwrap();
    assert!(content.contains("AUTO-GENERATED BY TAKO"));
    assert!(content.contains("`tako gen`"));
...
#[test]
fn write_tako_gen_module_exports_tako_runtime_object() {
    let dir = TempDir::new().unwrap();
    write_tako_gen_module(dir.path()).unwrap();
...
#[test]
fn write_tako_gen_module_imports_from_browser_safe_runtime_entry() {
...
#[test]
fn write_tako_gen_module_guards_process_env_reads_for_browser_bundlers() {
...
#[test]
fn write_tako_gen_module_emits_plain_runtime_env_global_augmentations() {
    let dir = TempDir::new().unwrap();
    write_tako_gen_module(dir.path()).unwrap();
    let content = fs::read_to_string(dir.path().join("src").join("tako.gen.ts")).unwrap();
...
#[test]
fn write_tako_gen_module_is_idempotent() {
    let dir = TempDir::new().unwrap();
    let written_first = write_tako_gen_module(dir.path()).unwrap();
    assert!(written_first);
    let written_second = write_tako_gen_module(dir.path()).unwrap();
    assert!(!written_second);
}

#[test]
fn write_tako_gen_module_uses_js_app_root() {
    let dir = TempDir::new().unwrap();

    let written =
        write_tako_gen_module_for_adapter_and_app_root(dir.path(), BuildAdapter::Unknown, "app")
            .unwrap();

    assert!(written);
    assert!(dir.path().join("app").join("tako.gen.ts").exists());
    assert!(!dir.path().join("tako.gen.ts").exists());
}

#[test]
fn write_tako_gen_module_with_secrets_generates_typed_interface() {
    let dir = TempDir::new().unwrap();
...
#[test]
#[cfg(unix)]
fn write_tako_gen_module_includes_takochannels_block_when_channels_present() {
    let dir = TempDir::new().unwrap();
...
"#,
    ...
    write_tako_gen_module_for_adapter_with_bun(
        dir.path(),
...
#[test]
fn write_tako_gen_module_deletes_legacy_tako_dts() {
    let dir = TempDir::new().unwrap();
    ...
    assert!(dir.path().join("src").join("tako.gen.ts").exists());
}
