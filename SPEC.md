...
- In deploys, `tako-server` validates the request and loads the source before checking its origin transform cache. Successful transformed variants are cached on local disk under the system temp directory, such as `/tmp/tako-image-cache`. Cache keys include the source bytes and transform options, so changed source files produce new variants. The cache is best-effort, local to each server, and cleaned according to the host temp directory policy.
