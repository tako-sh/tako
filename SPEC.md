...
Set `local: true` in the workflow opts to force that workflow onto per-server local SQLite storage. In multi-server environments, every JS workflow source file must opt into `local: true` until remote workflow storage is implemented; otherwise deploy fails before build/deploy work starts. Local cron schedules run once per server and local enqueue uniqueness is scoped to each server.
