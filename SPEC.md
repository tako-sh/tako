...
Workflow storage currently uses local SQLite at `{data_dir}/apps/{app}/{env}/data/tako/workflows.sqlite`. Multi-server workflow deploys are blocked by default because local workflow state would be split across servers. If a deployed environment targets more than one server and the JS app has `<app_root>/workflows/`, deploy fails before build/deploy work starts unless every workflow definition opts into per-server local execution with `defineWorkflow(..., { local: true, ... })`. Local workflows use each server's own local queue and cron schedule set; cron runs once per server and enqueued work is not globally deduplicated.

Channel replay currently uses local SQLite at `{data_dir}/apps/{app}/{env}/data/tako/channels.sqlite`. Multi-server channel deploys are blocked by default because a publish on one server must be visible to subscribers and reconnecting clients on every server. `postgres_url` can be stored today but deploy still fails for shared channel/workflow storage until the Postgres backend is implemented. The intended remote backend will be selected through internal storage adapters: SQLite for local mode and Postgres for shared mode. Workflow tables will live in schema `tako_workflows`; channel tables will live in schema `tako_channels`. Channel messages will be inserted durably before fanout, and Postgres notifications may be used only as wakeups for other servers to pull and deliver those rows.
...
The command prompts for a Cloudflare API token and optionally asks when the token expires. `--expires-on` can provide the expiry directly. The token is encrypted in `.tako/secrets.json` under the selected environment's `credentials` object as `ssl.cloudflare`. For Let’s Encrypt wildcard DNS-01, create a Cloudflare User API token in My Profile > API Tokens; Account API tokens are rejected because Cloudflare does not accept them for the zone discovery call Tako needs. The token must allow Zone Read and DNS Write for the matching Cloudflare zone, and any token IP restriction must include each target server's egress IP. Deploy validates that Let’s Encrypt wildcard routes have an unexpired credential when expiry is known, warns when the credential expires within 30 days, decrypts the selected environment's token locally, sends the SSL binding during remote prepare, verifies Cloudflare zone read access from the target server, and `tako-server` stores it encrypted in server SQLite for that deployed app after the final deploy command starts.
...
- `{name}` is a provider credential name. Supported values: `ssl.cloudflare` and `postgres_url`.
...
1. Pre-deployment validation (secrets present and unexpired, S3 credentials present and unexpired for selected storage bindings and configured backup storage, provider credentials present and unexpired when Cloudflare SSL is selected or Let’s Encrypt routes include wildcards, multi-server channel deploys are blocked until shared Postgres storage exists, multi-server workflow deploys require every workflow definition to set `local: true` until shared Postgres storage exists, warnings for app/S3/provider credentials expiring within 30 days, server target metadata present/valid for all selected servers)
...
- DNS-01 challenges are supported for Let’s Encrypt wildcard certificates through Cloudflare DNS. Set up encrypted provider credentials with `tako credentials set ssl.cloudflare --env <env>`. Use a Cloudflare User API token with Zone Read and DNS Write for the matching zone. Deploy rejects Cloudflare Account API tokens locally, rejects missing or expired credentials for wildcard routes, warns when they expire within 30 days, verifies Cloudflare DNS zone access from each target server during remote prepare, stages the SSL binding for the final deploy command, and `tako-server` stores it encrypted per deployed app.
...
- Let’s Encrypt wildcard certificates are issued automatically via Cloudflare DNS-01 challenges when the deployed app environment has unexpired `ssl.cloudflare` credentials stored by `tako credentials`; each target server verifies the token can read the matching Cloudflare zone during remote prepare, the token must allow DNS Write for issuance, and deploy warns when those credentials expire within 30 days
...
Every channel publish is stored before delivery. In production, the local replay log is a SQLite DB scoped to the deployed app id (`{name}/{env}`), not to a release or instance. In local dev, replay is per registered app and in-memory for the current daemon process. The log is bounded by `replayWindowMs`, which defaults to 10 minutes. This makes single-server production channel delivery durable across short disconnects, laptop sleep, browser reloads, `tako-server` reloads, and rolling deploys without making channels the app's permanent history API. Multi-server production channel deploys require the future Postgres backend so every server can read the same replay log and receive cross-server fanout wakeups; those channel tables live in schema `tako_channels`. Apps that need canonical history still store it in their own database and use channel replay as the reconnect bridge.
...
Set `local: true` in the workflow opts to force that workflow onto per-server local SQLite storage. In multi-server environments, every JS workflow source file must opt into `local: true` until shared Postgres channel/workflow storage is implemented; otherwise deploy fails before build/deploy work starts. Local cron schedules run once per server and local enqueue uniqueness is scoped to each server. Channels do not have a local opt-out for multi-server deploys because channel publish/replay must reach subscribers connected to every server.
