...
- Public image responses use `Cache-Control: public, max-age=31536000, immutable`. This is the browser/CDN response cache policy and is separate from Tako's origin source and transform caches.
...
- In deploys, `tako-server` runs libvips resize and encode work in an isolated child process with bounded concurrency. Requests wait for an available worker slot; the worker execution timeout starts only after the transform begins. If the image worker crashes or times out, the server process stays up and the transform failure fallback rules apply.
